...
def plot_flowsheet_nx(graph, plot_with_stream_labels, add_positions=True):
    ...
    fig = plt.figure(figsize=(flowsheet_size * 18. / 6., flowsheet_size * 9. / 6.))
    ...
    if plot_with_stream_labels:
        ...
        #pos = nx.spring_layout(graph,k=1/sqrt(graph.number_of_nodes()))
        # pos = nx.planar_layout(graph,scale=15)
        # # pos = nx.spring_layout(graph,k=1/(0.1*(graph.number_of_nodes())))
        # nx.draw(graph, pos, with_labels=True, node_size= flowsheet_size * 5000., font_size= flowsheet_size* 2., node_color="#69d239")
        # pos = nx.spring_layout(graph,k=1/(math.sqrt(flowsheet_size)))
        # nx.draw(graph, pos, with_labels=True, node_size= flowsheet_size*3000., font_size= flowsheet_size*1., node_color="#69d239")
        # pos = nx.planar_layout(graph,scale=15)
        # # pos = nx.spring_layout(graph,k=1/(math.sqrt(flowsheet_size)))
        # pos = nx.spring_layout(graph,k = 1/math.sqrt(flowsheet_size))
        # nx.draw(graph, pos, with_labels=True, node_size = flowsheet_size* 3500., font_size= flowsheet_size*2., node_color="#69d239")
        # pos  = nx.forceatlas2_layout(graph)
        # pos =  nx.bfs_layout(graph)
        ...
        # elif graph.number_of_nodes() <= 50:
        #     pos = nx.kamada_kawai_layout(graph, scale=10)
        # else:
        #     pos = nx.spring_layout(graph, k=1.5 / math.sqrt(graph.number_of_nodes()), iterations=100)
        # nx.draw(graph, pos, with_labels=True, node_color="#69d239")
        # # Choose layout as before
        # if n <= 10:
        #     if nx.check_planarity(graph)[0]:
        #         pos = nx.planar_layout(graph, scale=10)
        #     else:
        ...
        #     pos = nx.kamada_kawai_layout(graph, scale=10)
        # else:
        #     pos = nx.spring_layout(graph, k=1.5 / math.sqrt(n), iterations=100)

        # # Dynamically set node size and font size based on number of nodes
        # # Node size example: between 300 (small graphs) and 50 (large graphs)
        # node_size = max(50, 3000 // n)  # e.g. if n=10, size=300; if n=100, size=30 (min 50)

        # # Label font size between 8 and 16
        # font_size = max(6, 16 - n // 10)  # reduces font size as n grows
        ...
        #     with_labels=True,
        #     node_color="#69d239",
        #     node_size=flowsheet_size* 3200.,
        #     font_size=flowsheet_size * 1.,
        #     font_color='black',
        #     font_weight='bold'
        # )
